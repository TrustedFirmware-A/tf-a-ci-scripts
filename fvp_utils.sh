...
rootfs_url="${DOWNLOAD_SERVER_TF_A_URL}/linux_boot/fvp/rootfs.ext4"
...
[fvp-quad-busybox-uboot]="$DOWNLOAD_SERVER_TF_A_URL/quad_cluster/Image"
)
...
get_ftpm_optee_bin() {
	url="$DOWNLOAD_SERVER_TF_A_URL/ftpm/optee/tee-header_v2.bin" \
		saveas="bl32.bin" fetch_file
	archive_file "bl32.bin"

	url="$DOWNLOAD_SERVER_TF_A_URL/ftpm/optee/tee-pager_v2.bin" \
...
get_dtb() {
	...
	case "$dtb_type" in
		"fvp-base-quad-cluster-gicv3-psci")
			...
			dtb_url="$DOWNLOAD_SERVER_TF_A_URL/quad_cluster/fvp-base-quad-cluster-gicv3-psci.dtb"
...
gen_fvp_yaml() {
    ...
    declare -A fvp_artefact_urls=(
        ...
        [flash0]="$(gen_bin_url flash0)"
        [flash1]="$(gen_bin_url flash1)"
